...
public class MapDisplayFragment extends Fragment implements MapEventsReceiver, IMyLocationConsumer {
    ...
    private void plotRoutes() {
        ...
        List<RoutePattern> allRP=new ArrayList<>();
        if (s!=null){

            for (Route r:RouteManager.getInstance()){
                if (r.hasStop(s)){
                   allRP.addAll(r.getPatterns());

                    }
                }

            }
            Set<Route> setR=new HashSet<>();
            for (RoutePattern aRP:allRP){

                for (int i=0;i<aRP.getPath().size()-1;i++){
                    LatLon p1=aRP.getPath().get(i);
                    LatLon p2=aRP.getPath().get(i+1);
                    if (rectangleIntersectsLine(northWest,southEast,p1,p2)){
                        Boolean setBefore=setR.contains(aRP.getRoute());
                        setR.add(aRP.getRoute());
                        GeoPoint gp1=new GeoPoint(p1.getLatitude(),p1.getLongitude());
                        GeoPoint gp2=new GeoPoint(p2.getLatitude(),p2.getLongitude());
                        ...
                        busRouteLegendOverlay.add(aRP.getRoute().getNumber());}



                        line.setColor(busRouteLegendOverlay.getColor(aRP.getRoute().getNumber()));
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
        ...
        for (Stop s:StopManager.getInstance())
        {
             if (rectangleContainsPoint(northWest,southEast,s.getLocn())) {
                 if (nearestStopMarker==null) {
                     ...
                     GeoPoint position = new GeoPoint(s.getLocn().getLatitude(), s.getLocn().getLongitude());
                     ...
                     if (nearestStopMarker.getRelatedObject()==s){
                         stopClusterer.add(nearestStopMarker);
                         nearestStopMarker.setTitle(s.getNumber() + " " + s.getName());
    ...
    private void updateMarkerOfNearest(Stop nearest) {
        ...
        if (nearest!=null){
            ...
            nearestStopMarker.setPosition(gp);
        ...
        if (nearest==null&&nearestStopMarker!=null)
        {
            nearestStopMarker=new Marker(mapView);
            nearestStopMarker.setIcon(stopIconDrawable);
        }
    ...
    private void handleLocationChange(Location location) {
        LatLon loc = new LatLon(location.getLatitude(),location.getLongitude());
        Stop nearest=StopManager.getInstance().findNearestTo(loc);
        updateMarkerOfNearest(nearest);
       locationListener.onLocationChanged(nearest,loc);
       // onLocationChanged(nearest,loc);
