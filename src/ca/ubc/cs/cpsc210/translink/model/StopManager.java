...
public class StopManager implements Iterable<Stop> {
    ...
    public static final LatLon DEFAULTLOCN = new LatLon(49.2,-123.1);
    ...
    public void addStop(Stop stop){
        stopMap.put(stop.getNumber(),stop);
    }
    ...
    public Stop findNearestTo(LatLon pt) {
        ...
        for (Stop s:stopMap.values()){
            double current = SphericalGeometry.distanceBetween(s.getLocn(),pt);
            if (current<dist){
